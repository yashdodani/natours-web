...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(password, user.password))) {
    return next(new AppError('Incorrct email or password alag wala', 401));
  }
...
exports.isLoggedIn = catchAsync(async (req, res, next) => {
  ...
  if (req.cookies.jwt) {
    // 1) Verification token (if someone has manipulated or token is already expired)
    ...
    res.locals.user = currentUser; // all the pugs have access to req.locals, this is sam eas passing options in render.
    return next();
  }
  next();
});
